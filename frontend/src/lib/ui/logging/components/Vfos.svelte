    ...
    {#if catState.select === 'VFO-A' || catState.select === ''}
        <div class="flex flex-row items-center gap-x-2">
            {#if isSplit}
                ...
                <VfoInput id='vfoa' bind:value={qsoState.cat_vfoa_freq} band={frequencyToBandFromDottedMHz(qsoState.cat_vfoa_freq)}/>
            {:else}
                <VfoBox label='VFO-A'/>
                <VfoInput id='vfoa' bind:value={qsoState.cat_vfoa_freq} band={frequencyToBandFromDottedMHz(qsoState.cat_vfoa_freq)}/>
            {/if}
        ...
        <div class="flex flex-row items-center gap-x-2">
            {#if isSplit}
                ...
                <VfoInput id='vfob' bind:value={qsoState.cat_vfob_freq} band={frequencyToBandFromDottedMHz(qsoState.cat_vfob_freq)}/>
            {:else}
